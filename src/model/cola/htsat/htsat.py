...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4,4), 
        ...
        # self.depths = depths
        self.depths = self.config.htsat_depth
        self.ape = ape
        self.in_chans = in_chans
        self.num_classes = num_classes
        # self.num_heads = num_heads
        self.num_heads = self.config.htsat_num_head
    ...
    def forward_features(self, x):
        ...
        if self.config.enable_tscam:
            ...
            # get latent_output
            sequence_output = torch.flatten(x, 2)
    ...
    def crop_wav(self, x, crop_size, spe_pos = None):
        ...
        for i in range(len(x)):
            ...
            max_crop_size = min(x.shape[2] - crop_pos, crop_size)
            tx[i][0][:max_crop_size] = x[i, 0, crop_pos:crop_pos + max_crop_size,:]
        return tx
        ...
        if infer_mode:
            ...
            if x.shape[2] > self.freq_ratio * self.spec_size:
                if self.training:
                    ...
                    for cur_pos in range(0, x.shape[2] - crop_size - 1 + overlap_size, overlap_size):
                    ...
                    # sequence_output = torch.zeros_like(output_dicts[0]["sequence_output"]).float().to(x.device)
                    seq_output_shape = output_dicts[0]["sequence_output"].shape
                    sequence_output = torch.zeros(len(output_dicts)*seq_output_shape[1], seq_output_shape[2]).float().to(x.device)
                    # sequence_output = torch.reshape(output_dicts[0]["sequence_output"])
                    for i, d in enumerate(output_dicts):
                        clipwise_output += d["clipwise_output"]
                        framewise_output += d["framewise_output"]
                        latent_output += d["latent_output"]
                        sequence_output[i*seq_output_shape[1]:(i+1)*seq_output_shape[1]] = d["sequence_output"].squeeze()

                    clipwise_output  = clipwise_output / len(output_dicts)
                    framewise_output = framewise_output / len(output_dicts)
                    latent_output = latent_output / len(output_dicts)
                    # sequence_output = sequence_output / len(output_dicts)
                    output_dict = {
                        'framewise_output': framewise_output, 
                        'clipwise_output': clipwise_output,
                        'latent_output': latent_output,
